...
//			return d < this.negativeSlime.size() / 2;
	...
	public static void empty() {
		...
		List<CPos> negative = generateSquare(new CPos(-size / 2, -size / 2), new CPos(size / 2, size / 2));
		negative.removeIf(positive::contains);
		...
		List<CPos> negative2 = generateSquare(new CPos(-size / 2, -size / 2), new CPos(size / 2, size / 2));
		negative2.removeIf(positive2::contains);

		Requirements requirements2 = new Requirements(positive2, negative2, version);
		findSlimeSeeds2(requirements, requirements2).parallel().forEach(System.out::println);
	}
